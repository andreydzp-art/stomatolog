# Лендинги nkolesnik.com

Статические страницы стоматолога (экспорт Claude Design). Без сборки и npm-зависимостей.

| URL | Услуга |
|-----|--------|
| `/` и `/luminir/` | Композитные виниры |
| `/vinir/` | Керамические виниры |
| `/karies/` | Лечение кариеса |
| `/implantacia/` | Имплантация |

Пересборка из архива: `node scripts/build-pages.mjs` (источник — `../extracted/stoma-import-3/Лендинги/`).
...
|--------------|------------|
| `index.html` | Главная (композитные виниры) |
| `luminir/`, `vinir/`, `karies/`, `implantacia/` | Отдельные лендинги по услугам |
| `scripts/build-pages.mjs` | Сборка страниц из HTML-экспорта |
